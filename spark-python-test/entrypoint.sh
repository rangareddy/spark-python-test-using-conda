...
log_info "PySpark Unit Testing Modules : ${MODULES_TO_TEST_ARRY[@]}"
    ...
    for python_version in "${PYTHON_VERSIONS[@]}"; do
        ...
        if conda env list | grep -q "$CONDA_ENV_NAME" ; then
            ...
            for spark_version in "${SPARK_VERSIONS[@]}"; do
                ...
                pip install pip setuptools wheel requests "$PIP_OPTIONS"
                if [[ "$python_version" == "2."* ]]; then
                    pip install cython "numpy==1.10.4" "pyarrow>=0.8.0,<0.10.0" "pandas >= 0.19.2" scikit-learn matplotlib "$PIP_OPTIONS"
                else
                    pip install -U numpy pyarrow pandas scikit-learn scipy matplotlib "$PIP_OPTIONS"
                fi
                log_info "Display the list of Python Packages Installed"
                log_info "$(pip list)"
